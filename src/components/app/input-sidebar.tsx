...
import type { ScenarioData, StationConfig } from '@/lib/types';
...
const transportItemSchema = z.object({
  ...
  originStation: z.string().min(1, 'Origen requerido'),
  destinationStation: z.string().min(1, 'Destino requerido'),
...
const formSchema = z.object({
  ...
  })).min(2, 'Se necesitan al menos 2 estaciones'),
  ...
  refuelConfig: z.object({
    enabled: z.boolean(),
    maxFlightDistance: z.coerce.number().min(1, 'Autonomía mínima de 1 km'),
  }).optional(),
  mapBackgroundUrl: z.string().optional(),
}).refine(data => {
  const ids = new Set(data.stations.map(s => s.id));
  return data.transportItems.every(
    p => ids.has(p.originStation) && ids.has(p.destinationStation)
  );
}, { message: 'Estación de origen o destino no existe en el escenario', path: ['transportItems'] })
  .refine(data => {
...
export function InputSidebar({ scenario, setScenario, onGeneratePlans, isLoading }: InputSidebarProps) {
  ...
  const watchedStations = form.watch('stations') as StationConfig[];
    ...
    form.trigger().then(isValid => {
      if (isValid) {
        ...
        const processedValues = {
          ...values,
          stations: values.stations as StationConfig[],
  ...
  // Estaciones activas para los dropdowns de origen/destino
  const activeStations = watchedStations ?? scenario.stations;
  const baseStation = activeStations.find(s => s.isBase) ?? activeStations[0];

  const getReadableErrors = () => {
    const list: string[] = [];
    const errors = form.formState.errors;

    if (errors.stations) list.push(`Estaciones: ${(errors.stations as any)?.message || 'Error en configuración de estaciones'}`);
    if (errors.helicopterCapacity) list.push(`Capacidad: ${errors.helicopterCapacity.message}`);
    if (errors.helicopterMaxWeight) list.push(`Peso Máx: ${errors.helicopterMaxWeight.message}`);
    if (errors.paxDefaultWeight) list.push(`Peso PAX por defecto: ${errors.paxDefaultWeight.message}`);
    if (errors.refuelConfig?.maxFlightDistance) list.push(`Combustible: ${errors.refuelConfig.maxFlightDistance.message}`);
                  ...
                  <>
                    ...
                    <div className="bg-card border rounded-lg shadow-sm overflow-hidden">
                      ...
                      <div className="grid grid-cols-3 gap-2 px-3 pb-3">
                        ...
                        <FormField control={form.control} name="paxDefaultWeight" render={({ field }) => (<FormItem><FormLabel className="text-[10px] uppercase font-semibold text-muted-foreground">Peso PAX (kg)</FormLabel><FormControl><Input type="number" className="h-9 font-medium bg-background text-sm" {...field} /></FormControl></FormItem>)} />
                      </div>
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem className="space-y-1 mt-1.5 animate-in fade-in slide-in-from-top-1 duration-200">
                                <FormLabel className="text-[10px] uppercase font-semibold text-muted-foreground">Autonomía máxima (km)</FormLabel>
                                <FormControl>
                                  <Input
                                    type="number"
                                    step="1"
                      ...
                      <div className="px-3 pb-3 space-y-3">
                        ...
                        <StationManager
                          ...
                          onChange={(newStations) => {
                            form.setValue('stations', newStations, { shouldDirty: true });
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="bg-card border rounded-lg shadow-sm overflow-hidden">
                      ...
                      <div className="space-y-3 px-3 pb-3">
                        {fields.map((field, index) => {
                          ...
                          return (
                            <div key={field.id} className={`flex flex-col gap-2.5 rounded-lg border p-3 relative group transition-colors ${isPax ? 'bg-blue-500/5 border-blue-500/20 hover:border-blue-500/40' : 'bg-amber-500/5 border-amber-500/20 hover:border-amber-500/40'}`}>
                              ...
                              {/* Origin / Destination selects using station names */}
                        ...
                        <Button type="button" variant="outline" className="w-full h-10 text-sm border-dashed hover:border-solid hover:border-primary/40 hover:bg-primary/5 transition-all" onClick={() => append({ id: crypto.randomUUID(), area: '', type: 'PAX', shift: 'M', priority: 'MEDIA', quantity: 1, originStation: baseStation?.id ?? '', destinationStation: activeStations.filter(s => !s.isBase)[0]?.id ?? '', weight: 80, description: '' })}>
                    ...
                    {history.length > 0 ? (
                      <Accordion type="single" collapsible className="w-full">
                        {history.map((histScenario, index) => {
                          ...
                          const dateStr = histScenario.id
                            ? new Date(histScenario.id).toLocaleString('es-ES', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })
                            : 'Sin fecha';
                          const stationCount = histScenario.stations?.length ?? 0;
                            ...
                            <AccordionItem value={`item-${index}`} key={histScenario.id}>
                              <AccordionTrigger className="hover:no-underline">
                                <div className="flex flex-col items-start gap-0.5 flex-1 text-left">
                                  ...
                                  <span className="text-xs text-muted-foreground">
                                    {dateStr} · {histScenario.transportItems.length} items · {stationCount} est.
                                  </span>
                                </div>
                              </AccordionTrigger>
                              <AccordionContent className='space-y-3 pt-1'>
                                ...
                                <div className='text-xs text-muted-foreground grid grid-cols-2 gap-x-4 gap-y-1 bg-background rounded-md p-2 border'>
                                  ...
                                  <div className="flex justify-between"><span>Estaciones</span><strong className="text-foreground">{stationCount}</strong></div>
                                  ...
                                  <div className="flex justify-between col-span-2 border-t pt-1 mt-1"><span>Combustible</span><strong className="text-foreground">{histScenario.refuelConfig?.enabled ? `⛽ Sí (${histScenario.refuelConfig.maxFlightDistance} km)` : '❌ No'}</strong></div>
                                </div>
